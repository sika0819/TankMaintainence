...
public class TextEffectsManager : EditorWindow
{
    ...
    [MenuItem ("Window/TextFX/TextFX Manager")]
    static void Init ()
	{
	...
	void DrawEffectEditorPanel()
	{
		...
		for(int action_idx=0; action_idx < selected_animation.NumActions; action_idx++)
		{
			...
			if(action.FoldedInEditor)
			{
				...
				{
					// Backwards compatibility step for letter anchoring. Initialise anchor value to legacy enum equivalent
					if(action.m_letter_anchor_start < 0)
						action.InitialiseLetterAnchorType();
