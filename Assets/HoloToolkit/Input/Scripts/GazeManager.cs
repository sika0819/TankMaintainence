...
namespace HoloToolkit.Unity
{
    ...
    public partial class GazeManager : Singleton<GazeManager>
    {
       ...
       // public LayerMask[] RaycastLayerMask = new LayerMask[] { Physics.DefaultRaycastLayers };
        public LayerMask RaycastLayerTest;// =  Physics.DefaultRaycastLayers ;
       
        // public LayerMask[] RaycastLayerMask = new LayerMask[] { Physics. };
        ...
        private void Update()
        {
            ...
            // 重要  gazeOrigin = Camera.main.transform.position;
            // 重要  gazeRotation = Camera.main.transform.rotation;
            // 重要  if (GazeStabilization != null)
            // 重要  {
            // 重要      GazeStabilization.UpdateHeadStability(gazeOrigin, gazeRotation);
            // 重要  }           
            UpdateRaycast();
        ...
        private void UpdateRaycast()
        {
            ...
            if (GazeStabilization != null)
            {
                ...
                gazeOrigin = GazeStabilization.StableHeadPosition;
                gazeDirection = GazeStabilization.StableHeadRay.direction;
            }         
            ...
            RaycastHit? hit = PrioritizeHits(Physics.RaycastAll(new Ray(gazeOrigin, gazeDirection), MaxGazeDistance, -1));

            // Update the HitInfo property so other classes can use this hit information.


            //原来正确 HitInfo = hitInfo;
